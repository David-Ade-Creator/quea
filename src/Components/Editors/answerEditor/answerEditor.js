...
class ImageBlot extends BlockEmbed {
    ...
    static create(value) {
        ...
        imgTag.setAttribute('max-height', '350px');
        return imgTag;
    }
...
class VideoBlot extends BlockEmbed {
    ...
    static create(value) {
        if (value && value.src) {
            ...
            videoTag.setAttribute('max-height', '350px');
            ...
            iframeTag.setAttribute('max-height', '550px');
            return iframeTag;
        }
    }
            ...
            axios.post('/api/q3/upload/s3', formData, config)
                .then(response => {
                    if (response.data) {
                        ...
                        let range = quill.getSelection();
                        let position = range ? range.index : 0;
                        quill.insertEmbed(position, "video", { src: response.data , title: response.data.fileName });
                        quill.setSelection(position + 1);
