...
export default function AnswersListCard({ answers, user, answerLike,questionId}) {
  return (
    <div>
      {answers.map((answer) => {
        return (
          questionId ? (answer.questionId?._id === questionId) && <Card style={{ width: "100%", marginTop: "6px" }} key={answer._id}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <Postlike answer={answer} user={user} answerLike={answerLike}/>
            </div>
          </Card> : <Card style={{ width: "100%", marginTop: "6px" }} key={answer._id}>
            <Link to={`/profile/${answer?.writer?._id}`}>
              <Meta avatar={answer?.writer?.info?.photo ? <Avatar src={answer?.writer?.info?.photo}></Avatar>:<Avatar>{answer.writer?.firstname?.substring(0, 1)}</Avatar>}
               title={answer.writer?.firstname + " " + answer.writer?.lastname} />
            </Link>
